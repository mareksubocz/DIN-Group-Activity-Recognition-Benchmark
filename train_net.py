...
from model_with_ball import VolleyballNN
from config import *
...
def train_net(cfg):
    ...
    model = VolleyballNN(cfg, model=model)
...
def train_volleyball(data_loader, model, device, optimizer, epoch, cfg):
    ...
    for batch_data in data_loader:
        ...
        ball_pos = batch_data[4]

        # forward
        actions_scores,activities_scores=model((batch_data[0],batch_data[1]), ball_pos)
...
def test_volleyball(data_loader, model, device, epoch, cfg):
    ...
    with torch.no_grad():
        for batch_data_test in data_loader:
            ...
            ball_pos = batch_data_test[4]

            # forward
            actions_scores,activities_scores=model((batch_data_test[0],batch_data_test[1]), ball_pos, is_test=True)
